...
public class DatingView extends Composite {
	...
	private Button dateSendButton = new Button("Register the Product");
    ...
    final SuggestBox problemArea = new SuggestBox(createCountriesOracle());
    final SuggestBox product = new SuggestBox(createCountriesOracle());
    final SuggestBox price = new SuggestBox(createCountriesOracle());
    private Anchor myDatingGoalsLink = new Anchor("Track your Dating Goals");
    private Anchor cul = new Anchor("");
	DatingView(){
		//this.topBarViewListener = topBarViewListener;
		initWidget(datingTable);
		datingTable.setWidth("32em");
		datingTable.setCellSpacing(5);
		datingTable.setCellPadding(3);
		cellFormatter.setHorizontalAlignment(0, 1,
		        HasHorizontalAlignment.ALIGN_LEFT);
		datingTable.getFlexCellFormatter().setColSpan(1, 0, 2);
		datingTable.setWidget(1, 0, new HTML("<b>Set Your Dating Goals</b>"));
		datingTable.setWidget(1, 1, myDatingGoalsLink);
		addGoalType();
		/*dateSendButton.addStyleName("dateSendButton");
		dateSendButton.addClickHandler(new ClickHandler() {
	...
	public void addGoalType(){
	    ...
	    datingTable.setWidget(2, 0, new HTML("Current Problem Area"));
	    datingTable.setWidget(2, 1, problemArea);
	    datingTable.setWidget(3, 0, new HTML("Time You can Spend"));
		 datingTable.setWidget(3, 1, product);
		 datingTable.setWidget(4, 0, new HTML("Money you can Spend"));
		 datingTable.setWidget(4, 1, price);
		 datingTable.setWidget(5, 1, dateSendButton);
	   // datingTable.setWidget(2, 1, vdropBox);
	   // RootPanel.get("flexTable").add(datingTable);
	 ...
	 private void showCategory(ListBox listBox, final int category) {
		 ...
		 datingTable.setWidget(3, 0, new HTML("Product or Service Type"));
		 datingTable.setWidget(3, 1, product);
	 ...
	 private void showBudget(ListBox budgetListBox, int goalType, int goal) {
			...
			//goalTypeValue = goalTypes[goalType];
			  ...
			  datingTable.setWidget(4, 0, new HTML("Select Price Range"));
			  datingTable.setWidget(4, 1, budgetListBox);
...
public Anchor getMyDatingGoalLink(){
	return myDatingGoalsLink;
}
